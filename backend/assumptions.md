...
```
                        ...
                        └────┬─────┘
                             │ POST /{id}/pay (mock — moves directly to PAID)
                             ▼
                          ┌──────┐
                          │ PAID │
                          └──┬───┘
  ...
  PENDING  ──► EXPIRED     (auto-scheduler, after 15 min TTL)
```

> **Note on WAITING_PAYMENT**: State này tồn tại trong enum để chuẩn bị cho tích hợp payment gateway thật (Stripe/VNPay). Khi có gateway, flow sẽ là PENDING → WAITING_PAYMENT (initiate) → PAID (webhook confirm). Trong mock hiện tại, PENDING → PAID trực tiếp.
...
**Payment**
- `POST /bookings/{id}/pay` là mock endpoint chuyển thẳng `PENDING` → `PAID` trong một lần gọi. Trong production, flow đúng sẽ là: lần gọi đầu khởi tạo thanh toán (`WAITING_PAYMENT`), sau đó payment gateway gửi webhook xác nhận (`PAID`). Vì không có gateway thật trong 48h, hai bước được gộp thành một để Postman collection dễ test.
